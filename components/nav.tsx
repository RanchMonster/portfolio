...
export default function Nav() {
   ...
   return (
      <nav className="bg-bg-secondary border-border flex relative items-stretch border-b font-mono">
         ...
         {/* Desktop tabs */}
         <div className="hidden flex-1 items-stretch sm:flex">
            {links.map((link) => (
               <Link
                  key={link.href}
                  href={link.href}
                  className={`border-border flex items-center border-r px-4 py-2 text-sm transition-colors ${isActive(link.href)
               ...
               <div className="flex flex-col">
                  {links.map((link) => (
                     <Link
                        ...
                        className={`border-l-2 px-4 py-3 text-sm transition-colors ${isActive(link.href)
